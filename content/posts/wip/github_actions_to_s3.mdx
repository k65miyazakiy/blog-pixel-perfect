...
S3の更新方法としてはプッシュをトリガーに起動したGithub Actionsを用いてーというところまでは大雑把に決めていましたが、GithubからAWSのリソースにアクセスする方法についてアクセスキーを使うのはなんだかなといったところでした。

紆余曲折の結果、GithubからAWSへOpenID Connectを構成する方法があったことを知り、これを利用することでパイプライン上に認証情報を載せずにS3の更新を行うアーキテクチャを構成しました。
OpenID Connectを利用したGithub Actionsからクラウドリソースへのアクセスについては、 [OpenID Connect を使ったセキュリティ強化について](https://docs.github.com/ja/actions/security-for-github-actions/security-hardening-your-deployments/about-security-hardening-with-openid-connect) にも詳しく紹介があります。
...
- クラウドのシークレット情報が不要
  - 「Githubのこのレポジトリ、ブランチに関連するActionにはこの権限を付与する」といった設定を行うことになります。
- トークンの自動ローテート
  - クラウドリソースのアクセスには短命のトークンが利用されるため、トークンの漏洩によるリスクを軽減できます。

などです。シークレットキーに関わらず、認証情報をどこかに載せるのは避けたい気持ちがあったのでこの方法を採用しました。envなどは基本的にignoreしていますが、ヒューマンはエラーするものなので。

とまあメリットも多い構成ですが、その道中少々苦労する箇所があったので、本稿では以下、手順の説明を書き記しておきます。
...
なので具体的にはこの手順では、前者ではGithub Actionsで処理したいリソースと操作を（例えばS3への更新権限）指定して、後者ではこのレポジトリに登録されたGithub Actionsを信頼する、という設定を行います。

### 1-1. IDプロパイダの登録

OIDCなので、IDプロパイダが必要です。まずはこれをAWSに登録します。IDプロパイダという用語になじみがない場合、意味的には認証情報レジストリなどと読み替えてもらってもよいです。
AWSのIAMではデフォルトではGithubはIDプロバイダとして登録されていないので、まずはGithubをIDプロバイダとして登録します。
...
次に、先ほど登録したIDプロバイダを信頼するロールを作成します。
...
思わぬセキュリティホールを生まないよう、セキュリティの一般的なベストプラクティスに、従い必要最小限の認可対象を選択するとよいでしょう。"
/>

AWSでの設定は以上です。ここまでくれば後の設定は簡単です。

## 2. Github Actionsの設定

これまでの設定で、Github ActionsはAWSへ一時的なトークンを要求可能になりました。
このトークンにはIAMロールのポリシーが紐づいており、AWSのリソース操作の時に本トークンを併せて送ることでAWSリソースの操作が可能になります。

言葉で書くとちょっと長ったらしいですが、Github Actionsワークフローでは簡単にこれを設定できます。
