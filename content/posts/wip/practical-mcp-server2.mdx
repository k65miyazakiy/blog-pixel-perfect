...
**Capability設計、つまりどのようなTool（やその他）を提供するかどうかは利用用途やAIモデル、またサーバーが接続するコンテキストが提供しているアクセス方法によって異なる**でしょう。
...
もし、AIモデルが入力値を正しく生成できるかどうか不安な場合は、より入口を絞った形式でCapabilityを提供することも可能です。
...
本クライアントでは、list_tables（引数なし）や、describe_table（引数にテーブル名のみを指定可能）で、より入口を絞った形式でのエンドポイントを提供しています。
