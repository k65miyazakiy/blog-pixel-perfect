...
自動化されたアプローチを取ることで作業ワークフローは大きく改善**できます。
...
<Caution message="厳密には「クライアントが何を出来るか」もCapabilityに含まれるのでサーバーのみの話ではないですが、MCPサーバー開発者としてはサーバーの機能だと捉えておけば十分です" />
...
初期化フェーズでクライアントに対してCapabilityを提供しているので、そのうちのどれを利用するか、という命令がクライアントを経由してAIモデルからで渡ってきます（この命令はJSON形式で、MCPで定義されます）。
...
サーバーで宣言可能なCapabilidyの一覧は[Capability Negotiation](https://modelcontextprotocol.io/specification/2025-03-26/basic/lifecycle)で挙げられている通りです。
...
つまり、そもそも**次のアクションでどのツールを呼び出すか、を判断する際にDescriptionの内容は重要な役割を果たします。**
...
これは[SpecificationのMessages](https://modelcontextprotocol.io/specification/2025-03-26/basic#messages)に記載の通り、
すべてJSON RPC2で定義されたメッセージフォーマットを使用します。

これは実際に見てみるとシンプルで直感的です。`@modelecontextprotocol/servers`で提供される`filesytems`サーバーを利用した場合の通信メッセージの例を見てみましょう。
